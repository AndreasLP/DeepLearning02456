...
data_cartpole[0] = DownSampleTo84(torch.tensor(data_cartpole[0][:, 6:9, :, :]) / 255.)
data_cartpole[1] = DownSampleTo84(torch.tensor(data_cartpole[1][:, 6:9, :, :]) / 255.)

torch.manual_seed(0)
permuted_indicies1 = torch.randperm(data_cartpole[0].shape[0])
permuted_indicies2 = torch.randperm(data_cartpole[0].shape[0])
input_frames = data_cartpole[0][permuted_indicies1, :, :, :]
print(input_frames.shape)
target_frames = data_cartpole[1][permuted_indicies2, :, :, :]
...
class KeypointLearning(nn.Module):
    def __init__(self, ImageEncoder_channels: int = 32,
        ...
        device = 'cuda'
        self.ImageNet = input_image_encoder(in_channels=3).to(device)
        self.resolution_in = self.ImageNet(torch.randn((1,3,resolution_out,resolution_out)).to(device))[1].shape[-1]
        self.KeyNet = key_net(num_keypoints=n_heatmaps, in_channels=3).to(device)
